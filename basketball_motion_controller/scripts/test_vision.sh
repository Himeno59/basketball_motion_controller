...
# new-window fix-centroid "${SOURCE_SCRIPT} && rosrun basketball_motion calc-centroid.py"
new-window object_trajectory_estimator "${SOURCE_SCRIPT} && roslaunch basketball_motion object_trajectory_estimator.launch"
...
# new-window rosbag "${SOURCE_SCRIPT} && rossetjaxon_red && roscd basketball_motion/rosbag "
# sleep 1.0
# new-window hsi_color_filter "${SOURCE_SCRIPT} && rossetjaxon_red && roslaunch jsk_pcl_ros hsi_color_filter.launch basketball_color:=orange ball_detect:=true"
# new-window calc-pointcloud-centroid "${SOURCE_SCRIPT} && rossetjaxon_red && roslaunch jsk_pcl_ros_utils sample_centroid_publisher.launch INPUT:=/orange/hsi_output"
# # new-window fix-centroid "${SOURCE_SCRIPT} && rosrun basketball_motion calc-centroid.py"
# new-window object_trajectory_estimator "${SOURCE_SCRIPT} && rossetjaxon_red && roslaunch basketball_motion object_trajectory_estimator.launch"
# new-window ros-topic "${SOURCE_SCRIPT} && rossetjaxon_red && rostopic list"
